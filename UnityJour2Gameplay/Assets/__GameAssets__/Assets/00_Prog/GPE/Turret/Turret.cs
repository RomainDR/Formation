...
public class Turret : GPEBase
{
    ...
    private void Update()
    {
        if (!playerTarget || playerTarget.CurrentLife <= 0) return;
        transform.LookAt(playerTarget.transform);
        _timeElapsed += Time.deltaTime;
        if (!(_timeElapsed >= shootTime)) return;
        _timeElapsed = 0;
        Instantiate(projectileInstance, transform.position + transform.forward, transform.rotation).name =
            "Projectile " + Guid.NewGuid();
